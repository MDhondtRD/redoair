...
import com.realdolmen.redoair.ejb.*;
...
public class BookingRepositoryTest extends DataPersistenceTest {
    ...
    public void after() {
        ...
        for (Airport a : aRepo.getAllAirports())
            aRepo.getEntityManager().remove(a);
        aRepo.getEntityManager().flush();
        populated = false;
    }
    ...
    public void initialiseRepositories() {
        ...
        aRepo = new AirportRepository();
        aRepo.setEntityManager(entityManager());
    }
    ...
    public void populateDatabase() {
        ...
        for (int i = 1; i <= numberOfFlights; i++)
            fRepo.createFlight(new Flight("MD" + (6000 + i), aRepo.getAllAirports().get(0), aRepo.getAllAirports().get(1), LocalDateTime.now(), 25, 9.99));
        fRepo.getEntityManager().flush();
    ...
    public void creditCardValidation(){
        assertTrue(repo.isValidCreditCardNumber("5545057015667422"));
        assertFalse(repo.isValidCreditCardNumber("5545057015667423"));
        assertFalse(repo.isValidCreditCardNumber("1234567890123456"));
    }
}
